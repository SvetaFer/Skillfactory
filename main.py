...
RULES = ["Добро пожаловать в игру морской бой! \n"
         "Немного правил:\n"
         "Добавьте свои корабли: один 3ехпалубный, два 2ухпалубных, четыри 1палубных.\n"
         "Введите в формате координат начало и конец корабля: сначала строка, потом столбец.\n"
         "Корабли не должны касаться друг друга и должны быть на расстоянии не менее 1 клетки.\n"
         "■ - ваш корабль, X - подбитый корабль, Т - промах, ◦ - контур вокруг корабля \n"]
...
class Ship:
    def __init__(self, begin: Dot, end=None):
        ...
        try:
            ...
            print(error)
            raise ActionWasNotDone("Повторите попытку")
    ...
    def check_correct_ship(self):
        if not ((self.begin.x == self.end.x
                ...
                and len(self) <= 3):
            raise IncorrectShip("Корабль некорректный")
...
class Board:
        ...
        try:
            ...
            for dot in ship.all_dots:
                if not Board.is_dot_on_board(dot):
                    raise OutOfBoard("Корабль за пределами поля")
                if not isinstance(self.board_list[dot.x][dot.y], Dot):
                    raise FieldIsOccupied("Точка заняа")
    ...
    def can_we_add_another_ship(self, ship: Ship):
        ...
        if (((len(ship) == 3 and
            ...
            raise AllTheseShipsAreUsed("Все корабли поставлены")
    ...
    def shoot_at_ship(self, dot: Dot, index: int):
        ...
        if ship.lives == 0:
            ...
            if ship.lives == 0:
                ...
                self.add_contour(ship)
            raise YouHitTheTarget("Ранили! Стрелйте еще раз!")
...
class PlayerHuman(Player):
    ...
    def fill_board(self):
        ...
        while ships_amount < 7:
            ...
            coord = input("Добавьте корабль и введите СНОВА, если хотите переставить корабль")
            ...
            try:
                if len(coord) != 2 and len(coord) != 4:
                    raise InputRecognitionError("Повторите попытку")
                if len(coord) == 2:
                    coord = coord * 2
                if not(coord[0].isalpha() and coord[1].isdigit() and
                       coord[2].isalpha() and coord[3].isdigit()):
                    raise InputRecognitionError("Повторите попытку")
                dot_1 = Dot(ord(coord[0]) - 65, int(coord[1]) - 1)
                dot_2 = Dot(ord(coord[2]) - 65, int(coord[3]) - 1)
    ...
    def human_shoot() -> Dot:
        ...
        if len(move) != 2 and not (move[0].isalpha() and move[1].isdigit()):
            raise InputRecognitionError("Ошибка")
        dot = Dot(ord(move[0]) - 65, int(move[1]) - 1)
        if not Board.is_dot_on_board(dot):
            raise OutOfBoard("Точка за пределами поля")
        return dot
    @staticmethod
...
class PlayerComputer(Player):
    ...
    def comp_shoot(self, board: Board):
        if self.next_shoot_dots:
            ...
            if (isinstance(board.board_list[x][y], Dot) and
...
class Game:
    def __init__(self):
        self.human_first = random.randrange(2)
    ...
    def play_again():
        ...
        if answer == "НЕТ":
            print("Буду ждать тебя в следующий раз!")
        elif answer == "ДА":
            main()
        else:
            print("Повторите еще раз, не понял)")
            Game.play_again()
